...
#   kernelspec:
#     display_name: Python (object_ctrl Renku RF-DETR)
#     language: python
#     name: object-ctrl-renku-rfdetr
# ---
...
#    No Roboflow account or new dataset download is needed.
...
from object_ctrl import PROJECT_ROOT, aligned_print, configure_stdio_relative_path
from object_ctrl.dataset.rfdetr import prepare_coco_dataset, summarize_dataset
from object_ctrl.evaluation import write_comparison
from object_ctrl.platforms import rfdetr as rfdetr_platform
...
cache_paths = rfdetr_platform.configure_caches(PROJECT_ROOT)
...
# the Renku shell. For interactive use, edit the dictionary below. Its values
# take precedence without changing the kernel environment.
#
# | Purpose | `RUN_OVERRIDES` |
# | --- | --- |
# | Full training (default) | `{}` |
# | Short Renku GPU smoke check | `{"smoke_run": True}` |
...
#
# A **Renku smoke run** uses two epochs with
...
SOURCE_DATASET = (
    PROJECT_ROOT / "datasets" / "composed" / "coco_basketball_11501_1156_1395"
)
dataset_name = "basketball_11501_1156_1395"
if settings.smoke_run:
    dataset_name += "_smoke"
DERIVED_DATASET = PROJECT_ROOT / "data" / "processed" / "rfdetr" / dataset_name
...
# %%
run_dir = rfdetr_platform.prepare_run(PROJECT_ROOT, settings, manifest, runtime)
...
# %%
comparison_dir = (
    PROJECT_ROOT / "outputs" / "comparisons" / "basketball_large_dataset" / run_dir.name
)
if settings.smoke_run:
...
display(Markdown(
    ...
    f"Artifacts are saved in `{run_dir.relative_to(PROJECT_ROOT)}`."
))
